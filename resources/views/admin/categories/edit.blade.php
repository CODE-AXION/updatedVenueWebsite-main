            ...
            @include('layouts.sidebar')
                ...
                <div class="ml-4 mt-16 w-9/12">
                    <form enctype="multipart/form-data" action="{{route('category.update',$category->id)}}" method="POST">
                        @csrf
                        @method('PUT')
                    <h1 class="text-3xl mt-4 mb-8"> Update Category: <br>
                    {{$category->name}}
                    </h1>
                    <div class="mb-6">
                        <label for="text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your Category Name</label>
                        <input type="text" value="{{old('name',$category->name ?? null)}}" name="name" id="email" class="bg-gray-50 w-80 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 " placeholder="Wedding, Kitty kat, parties, lol deaths haha" >
